...
if [ ! -f "${SITE_PATH}/settings.php" ]; then
    ...
    drush site:install standard \
      ...
      --site-name="Content Insights Demo" \
      --account-name=admin \
      --account-pass=admin \
      -y
...
# 5. Enable module
echo "Enabling Content Insights Module..."
drush en content_insights -y
echo "Content Insights Module enabled."

# 6. Add content_insights_dashboard_block to a content region in the Claro theme
echo "Adding Content Insights Dashboard block to Claro Content region..."
drush php:eval "use \Drupal\block\Entity\Block; Block::create(['id' => 'claro_contentinsightsdashboard', 'theme' => 'claro', 'region' => 'content', 'plugin' => 'content_insights_dashboard_block', 'settings' => ['label' => 'Content Insights Dashboard', 'label_display' => 'visible'], 'visibility' => ['request_path' => ['id' => 'request_path', 'negate' => false, 'pages' => \"/node/add/*\r\n/node/*/edit\"], 'response_status' => ['id' => 'response_status', 'negate' => false, 'status_codes' => [200]]]])->save();"
echo "Block Content Insights Dashboard added to Claro Content region."
